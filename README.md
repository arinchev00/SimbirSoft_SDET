...
#### Здравствуйте!
##### Меня зовут Илья. 
Я не имею много знаний в автотестировании, т.к. заканчивал курсы только по ручному тестированию от  Skypro. Отдельно изучал основы яызка программирования Python на Stepik (жалко что не Java, сейчас бы очень пригодилось). А автоматизированное изучил буквально за пару недель для выполнения Вашего тестового задания). Так что я являюсь можно сказать полностью __самоучкой__. Просмотрел кучу видео и в результате что-то у меня получилось, а что-то не совсем, но об этом по порядку.


#### Что удалось осуществить из ТЗ
1. При выполенении мне удалось использовать IntelliJ IDEA, Selenium Webdriver, TestNG, Maven, GIT, поиск по селекторам css, xpath, id.
2. Тест-кейсы я разделил в своем коде на 2 отдельных класса (_Test1_ - это первый тест-кейс; _Test2_ - второй) и осуществил прогон их по порядку, записав на видео свой результат (его вы можете увидеть чуть ниже).
3. Произвести запуск тестов с помощью отдельной xml (_testng.xml_).
4. Выгрузить коллекцию полностью напрямую из IntelliJ IDEA в GitHub.

###### Все тест-кейсы закончились согласно ожидаемому результату.

#### Что не удалось осуществить из ТЗ

1. Не смог полностью разобраться и применить на практике паттерн проектирования Page Object. Я посмотрел различные видео, понял что там нужно разделить логику и реализацию отдельно (по разным классам), но к сожалению четкой последовательности действий я не нашел для Java и Selenium. Пробовал сделать по аналогии: из-за чего у меня то открывалось несколько окон браузера и он не понимал где искать элементы, то вообще ничего не получалось..
